...
-- ============================================================
-- RPC: get_ap_periodo_cobertura — período real de cobertura dos dados
-- ============================================================
-- Alimenta o componente PeriodBadge no frontend.
-- Retorna o período real das imagens PRODES e o gap coberto pelo DETER.
-- Distinção obrigatória: PRODES = confirmado | DETER = alerta provisório
-- ============================================================
CREATE OR REPLACE FUNCTION get_ap_periodo_cobertura(
    p_ano SMALLINT DEFAULT 2024
)
RETURNS JSON
LANGUAGE sql
...
COMMENT ON FUNCTION get_ap_periodo_cobertura IS
    'Período real de cobertura dos dados: datas das imagens PRODES + gap DETER.
     Alimenta o PeriodBadge no frontend. Retorna a execução mais recente com status sucesso/dry_run.';

...
def _delete_all(sb, table: str, pk_col: str) -> None:
    ...
    duplicatas — inflando os hectares totais (2x-6x observados em produção).
...
def upload_geodataframe(
    ...
    Args:
        ...
        if_exists: "upsert" (padrão) ou "replace" (delete-all + insert) — use
                   "replace" para tabelas recomputadas por inteiro cuja chave de
                   conflito não é estável entre runs (ex.: alertas_classificados).
